...
export default Moderators = (props) => {
  ...
  const onItemClick = (singleUser) => {
    ...
    setCertificationDate(moment(singleUser.certificateDate).format());
  ...
  useEffect(() => {
    users.map((singleUser) => {
      if (singleUser.userType === 1 && singleUser.isActive === true) {
        setSelectedMember(singleUser);
        setName(singleUser.name);
        setLastName(singleUser.lastName);
        setPhone(singleUser.Phone);
  ...
  const renderTeamMembers = ({item}) => {
    return item.userType == 1 && item.isActive === true ? (
